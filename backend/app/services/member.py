...
def adjust_points(db: Session, member_id: int, body: PointsAdjust):
    ...
    member = get_member(db, member_id)
    if member.points + body.delta < 0:
        raise HTTPException(status_code=400, detail="积分不足")
    return member_repo.adjust_points(db, member, body.delta)


def record_consumption(db: Session, member_id: int, amount):
    """下单结账后回写会员消费：累计消费、到店次数、会员等级、积分（需求文档 3.7）"""
    member = get_member(db, member_id)
    member.total_spend = member.total_spend + amount
    member.visits = member.visits + 1
    member.level = _level_for_spend(member.total_spend)
    member_repo.update_member(db, member)
    return member_repo.adjust_points(db, member, int(amount))
